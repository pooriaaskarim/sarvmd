...
String emit(PageConfig config, PageLayout layout) {
  ...
  for (final system in layout.systems) {
    ...
    // For single line, we usually don't have it in the UI, so let's match UI.
    if (config.layoutType == LayoutType.doubleLine) {
      ...
      final sysBottomPdfY = pageHBp -
          _mmToBp(system.staves.last.topY + config.staffConfig.staffHeightMm);
      
      // Barline thickness should match connector bolder look (e.g. 2.5x)
    ...
    for (var i = 0; i < system.staves.length; i++) {
      ...
      if (clef != null) {
        final topLinePdfY = pageHBp - _mmToBp(staff.topY);
        final anchorPdfY = topLinePdfY - (5 - clef.anchorLine) * lineGapBp;
        ...
        final baselinePdfY = anchorPdfY - anchorSp * lineGapBp - microOffsetBp;
        ...
        final scale = (lineGapBp * 4.0) / upem;
